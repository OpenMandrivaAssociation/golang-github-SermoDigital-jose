...
%global project         SermoDigital
%global repo            jose
# https://github.com/SermoDigital/jose
%global provider_prefix %{provider}.%{provider_tld}/%{project}/%{repo}
%global import_path     %{provider_prefix}
%global commit          f6df55f235c24f236d11dbcf665249a59ac2021f
%global shortcommit     %(c=%{commit}; echo ${c:0:7})

Name:           golang-%{provider}-%{project}-%{repo}
Version:        1.1
Release:        3%{?dist}
Summary:        A comprehensive set of JWT, JWS, and JWE libraries for Go (golang)
# Detected licences
...
Provides:      golang(%{import_path}/crypto) = %{version}-%{release}
...
for file in $(find . -iname "*_test.go" -o -wholename "./jws/test/*") ; do
    dirprefix=$(dirname $file)
    install -d -p %{buildroot}/%{gopath}/src/%{import_path}/$dirprefix
    cp -pav $file %{buildroot}/%{gopath}/src/%{import_path}/$file
